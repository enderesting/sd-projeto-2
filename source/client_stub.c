...
int rtable_put(struct rtable_t *rtable, struct entry_t *entry,int* connected) {
    ...
    if (res->opcode == MESSAGE_T__OPCODE__OP_BAD) {
        ...
        printf("Your function call was given incorrect and/or missing"
               "parameters\n");
        return -1;
    }
...
struct entry_t **rtable_get_table(struct rtable_t *rtable,int* connected) {
    ...
    struct entry_t** entry_arr =
        (struct entry_t**) calloc(res->n_entries + 1, sizeof(struct entry_t*));
...
struct statistics_t *rtable_stats(struct rtable_t *rtable,int* connected) {
    ...
    struct statistics_t* stat =
        (struct statistics_t*) calloc(1, sizeof(struct statistics_t));
